...
def main(feature_set_key):
    ...
    for current_model_class in search_models:
        ...
        if current_model_class == CatBoostModel:
            ...
            search_spaces = {'criterion': ['gini', 'entropy'], 'max_depth': [3, 4, 6, 10], 'n_estimators': [100, 200, 500], 'max_features': ['sqrt', 'log2', 0.2, 0.5, 0.8], 'min_samples_split': [2, 5, 20, 50]}
        else:
            logger.warn("Model not found. Continue")
            continue

        logger.info("Hyperparam search for model {} started".format(current_model.get_name()))
        ...
        logger.info("Hyperparam search for model {} finished after {}  seconds".format(current_model.get_name(), end - start))
